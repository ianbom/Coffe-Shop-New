...
class ProductSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($i = 0; $i < 6; $i++) { // Adjust the loop count if necessary
            try {
                // Generate a fake image using an alternative image source
                $imageUrl = 'https://placeimg.com/640/480/drink?random=' . $i; // Alternative image URL
                $imageContents = @file_get_contents($imageUrl);

                // Fallback to a local placeholder if the image could not be fetched
                if ($imageContents === false) {
                    $imageContents = Storage::disk('public')->get('placeholder.jpg'); // Use a local placeholder image
                }
                ...
                // Handle exceptions such as network errors or file permission issues
